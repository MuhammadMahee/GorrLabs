<script lang="ts">
	...
	const getTerminalInfo = (): { serverId: string; baseUrl: string } | null => {
		...
		if (systemMatch) {
			// For system terminals, WS goes through the Arkive backend proxy
			return { serverId: systemMatch.id, baseUrl: ARKIVE_API_BASE_URL };
		}
	...
	const connect = async () => {
		...
		try {
			...
			if (info.serverId === '__direct__') {
				...
				// System terminal — proxy through Arkive backend
				const base = info.baseUrl.replace(/\/$/, '');
				authToken = token;
