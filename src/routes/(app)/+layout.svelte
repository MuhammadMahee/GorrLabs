<script lang="ts">
	...
	import { ARKIVE_VERSION, ARKIVE_API_BASE_URL } from '$lib/constants';
	...
	const setToolServers = async () => {
		...
		if (systemTerminals.length > 0) {
			...
			const terminalEntries = systemTerminals.map((t) => ({
				id: t.id,
				url: `${ARKIVE_API_BASE_URL}/terminals/${t.id}`,
				name: t.name,
...
{#if $user}
	<div class="app arkive-page relative">
		<div
			class="arkive-shell text-gray-700 dark:text-gray-100 bg-white dark:bg-gray-900 h-screen max-h-[100dvh] overflow-auto flex flex-row justify-end"
		>
			{#if !['user', 'admin'].includes($user?.role)}
				<AccountPending />
			{:else}
