<script>
	...
	const setupSocket = async (enableWebsocket) => {
		const _socket = io(`${ARKIVE_BASE_URL}` || undefined, {
			reconnection: true,
		...
		_socket.on('connect', async () => {
			...
			if (version !== null || deploymentId !== null) {
				if (
					($ARKIVE_VERSION !== null && version !== $ARKIVE_VERSION) ||
					($ARKIVE_DEPLOYMENT_ID !== null && deploymentId !== $ARKIVE_DEPLOYMENT_ID)
				) {
					await unregisterServiceWorkers();
			...
			if (deploymentId !== null) {
				ARKIVE_DEPLOYMENT_ID.set(deploymentId);
			}

			if (version !== null) {
				ARKIVE_VERSION.set(version);
			}
				...
				if (done) {
					...
					if ($isLastActiveTab) {
						if ($settings?.notificationEnabled ?? false) {
							new Notification(`${displayTitle} • Arkive`, {
								body: content,
								icon: `${ARKIVE_BASE_URL}/static/favicon.png`
							});
						}
					}
	...
	const channelEventHandler = async (event) => {
		...
		if ((!channel || isFocused) && event?.user?.id !== $user?.id) {
			...
			if (type === 'message') {
				...
				if ($isLastActiveTab) {
					if ($settings?.notificationEnabled ?? false) {
						new Notification(`${title} • Arkive`, {
							body: data?.content,
							icon: `${ARKIVE_API_BASE_URL}/users/${data?.user?.id}/profile/image`
						});
					}
				}
	...
	onMount(async () => {
		...
		if (backendConfig) {
			...
			await ARKIVE_NAME.set(backendConfig.name);
...
<svelte:head>
	<title>{$ARKIVE_NAME}</title>
	<link crossorigin="anonymous" rel="icon" href="{ARKIVE_BASE_URL}/static/favicon.png" />

	<meta name="apple-mobile-web-app-title" content={$ARKIVE_NAME} />
	<meta name="description" content={$ARKIVE_NAME} />
	<link
		rel="search"
