...
from arkive.utils.misc import get_message_list
from arkive.socket.main import get_event_emitter
from arkive.utils.audit import log_response_generation_stage, create_final_audit_record
from arkive.utils.confidence_scoring import calculate_confidence_score
from arkive.utils.response_composer import ResponseComposer
from arkive.models.chats import (
    ChatForm,
    ChatImportForm,
...
from arkive.models.tags import TagModel, Tags
from arkive.models.folders import Folders
from arkive.internal.db import get_session

from arkive.config import ENABLE_ADMIN_CHAT_ACCESS, ENABLE_ADMIN_EXPORT
...
from arkive.utils.access_control import has_permission
...
############################
# GenerateMessageCompletion (Week 4)
############################
class CompletionRequest(BaseModel):
    """Request to generate a complete response with audit logging."""
    model_id: str
...
@router.post('/{chat_id}/messages/{message_id}/complete', response_model=Optional[Dict[str, Any]])
async def generate_message_completion(
    chat_id: str,
    message_id: str,
    form_data: CompletionRequest,
    user=Depends(get_verified_user),
    db: Session = Depends(get_session),
):
    """
    Generate a complete response with full audit trail logging.

    Orchestrates the entire response pipeline:
    1. RETRIEVAL — fetch relevant documents
    2. REDACTION — detect and mask PII
    3. MODEL_CALL — invoke LLM with context
    4. RESPONSE_COMPOSITION — assemble final response
    5. FINAL — link all stages together

    Args:
        chat_id: ID of the chat
        message_id: ID of the user message
        form_data: CompletionRequest with model_id and optional sources
        user: Authenticated user
        db: Database session

    Returns:
        Dict with response, confidence, citations, redaction notice, and audit IDs
    """
    try:
        # ──────────────────────────────────────────────────────────────
        # Setup: Validate chat and message
        # ──────────────────────────────────────────────────────────────
        ...
        # ──────────────────────────────────────────────────────────────
        # Stage 1: RETRIEVAL — Fetch relevant documents
        # ──────────────────────────────────────────────────────────────

        retrieval_start = time.time()
        retrieval_audit_id = None
        retrieved_sources = form_data.retrieved_sources or []

        try:
            retrieval_audit_id = await log_response_generation_stage(
                user_id=user.id,
                message_id=message_id,
                stage='retrieval',
                inputs={
                    'user_message': user_message_content[:500],
                    'source_count': len(retrieved_sources),
                },
                outputs={
                    'sources_retrieved': len(retrieved_sources),
                    'total_source_quality': sum(
                        s.get('quality_score', 0) for s in retrieved_sources
                    ) if retrieved_sources else 0,
                },
                duration_ms=int((time.time() - retrieval_start) * 1000),
                status='success',
            )
            log.info(
                f'[complete] retrieval stage logged: '
                f'chat={chat_id} message={message_id} sources={len(retrieved_sources)}'
            )
        except Exception as e:
            log.warning(f'[complete] failed to log retrieval stage: {e}')

        # ──────────────────────────────────────────────────────────────
        # Stage 2: REDACTION — Log PII detection (already handled by policy_engine)
        # ──────────────────────────────────────────────────────────────

        redaction_start = time.time()
        redaction_audit_id = None
        redacted_entities = []  # PII redaction is handled by policy_engine separately

        try:
            # Log the redaction stage even though PII masking happens upstream
            redaction_audit_id = await log_response_generation_stage(
                user_id=user.id,
                message_id=message_id,
                stage='redaction',
                inputs={'source_count': len(retrieved_sources)},
                outputs={
                    'entities_detected': len(redacted_entities),
                    'note': 'PII redaction handled by policy_engine',
                },
                duration_ms=int((time.time() - redaction_start) * 1000),
                status='success',
            )
        except Exception as e:
            log.warning(f'[complete] failed to log redaction stage: {e}')

        # ──────────────────────────────────────────────────────────────
        # Stage 3: MODEL_CALL — Invoke LLM
        # ──────────────────────────────────────────────────────────────

        model_call_start = time.time()
        model_call_audit_id = None
        answer_text = ''  # Would be populated by LLM call

        try:
            # In production, this would call the actual LLM completion endpoint
            # For now, we log the attempt

            # Placeholder for LLM response generation
            answer_text = f'Response to: {user_message_content[:100]}...'

            model_call_duration = int((time.time() - model_call_start) * 1000)
            model_call_audit_id = await log_response_generation_stage(
                user_id=user.id,
                message_id=message_id,
                stage='model_call',
                inputs={
                    ...
                    'context_source_count': len(retrieved_sources),
                },
                outputs={
                    'response_length': len(answer_text),
                    'tokens_generated': len(answer_text.split()),
                },
                duration_ms=model_call_duration,
                status='success',
            )
            log.info(
                f'[complete] model_call stage logged: '
                f'chat={chat_id} message={message_id} '
                f'model={form_data.model_id} duration={model_call_duration}ms'
            )
        except Exception as e:
            log.warning(f'[complete] failed to log model_call stage: {e}')
        ...
        try:
            if model_call_audit_id:
                score_result = await calculate_confidence_score(
                    audit_log_id=model_call_audit_id,
                    retrieved_sources=retrieved_sources,
                    fact_check_results=form_data.fact_check_results or [],
                    classification_score=form_data.classification_score or 0.5,
                )
                if score_result:
                    confidence_score = score_result.get('final_confidence', 0.7)
                    confidence_breakdown = score_result.get('breakdown', {})
                    log.info(
                        f'[complete] confidence calculated: '
                        f'chat={chat_id} message={message_id} '
                        f'score={confidence_score:.2f}'
                    )
        except Exception as e:
            log.warning(f'[complete] failed to calculate confidence: {e}')

        # ──────────────────────────────────────────────────────────────
        # Stage 5: RESPONSE_COMPOSITION — Assemble final response
        # ──────────────────────────────────────────────────────────────

        composition_start = time.time()
        composition_audit_id = None

        try:
            composed_response = await ResponseComposer.compose(
                message_id=message_id,
                user_id=user.id,
                answer_text=answer_text,
                confidence_score=confidence_score,
                confidence_breakdown=confidence_breakdown or {
                    'interpretation': 'Confidence calculation complete'
                },
                citations=retrieved_sources,
                redacted_entities=redacted_entities,
                response_audit_log_id=model_call_audit_id,
                policy_decision_id=form_data.policy_decision_id,
            )

            if composed_response:
                composition_audit_id = await log_response_generation_stage(
                    user_id=user.id,
                    message_id=message_id,
                    stage='response_composition',
                    inputs={
                        'confidence_score': confidence_score,
                        'citation_count': len(retrieved_sources),
                    },
                    outputs={
                        'response_assembled': True,
                        'redacted': len(redacted_entities) > 0,
                    },
                    duration_ms=int((time.time() - composition_start) * 1000),
                    status='success',
                )
                log.info(
                    f'[complete] response_composition stage logged: '
                    f'chat={chat_id} message={message_id}'
                )
        except Exception as e:
            log.warning(f'[complete] failed to compose response: {e}')

        # ──────────────────────────────────────────────────────────────
        # Stage 6: FINAL — Link all stages together
        # ──────────────────────────────────────────────────────────────

        final_audit_id = None
        try:
            final_audit_id = await create_final_audit_record(
                message_id=message_id,
                user_id=user.id,
                policy_decision_id=form_data.policy_decision_id,
                retrieval_audit_id=retrieval_audit_id,
                redaction_audit_id=redaction_audit_id,
                model_call_audit_id=model_call_audit_id,
                composition_audit_id=composition_audit_id,
            )
            log.info(
                f'[complete] final audit record created: '
                f'chat={chat_id} message={message_id} audit_id={final_audit_id}'
            )
        except Exception as e:
            log.warning(f'[complete] failed to create final audit record: {e}')

        # ──────────────────────────────────────────────────────────────
        # Return Composed Response with Audit Trail
        # ──────────────────────────────────────────────────────────────

        if composed_response:
            # Add final audit ID to response
            composed_response['audit']['final_audit_id'] = final_audit_id
            ...
            log.info(
                f'[complete] message completion successful: '
                f'chat={chat_id} message={message_id} user={user.id}'
            )
            return composed_response
        else:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail='Failed to compose response',
            )
        ...
        log.exception(
            f'[complete] unexpected error: chat={chat_id} message={message_id} error={e}'
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
