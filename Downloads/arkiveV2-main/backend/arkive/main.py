...
from arkive.utils import logger
from arkive.utils.audit import AuditLevel, AuditLoggingMiddleware
from arkive.utils.logger import start_logger
from arkive.socket.main import (
    MODELS,
    app as socket_app,
...
from arkive.internal.db import ScopedSession, engine, get_session

from arkive.models.functions import Functions
from arkive.models.models import Models
from arkive.models.users import UserModel, Users
from arkive.models.chats import Chats
...
from arkive.env import (
    ...
    ARKIVE_BUILD_HASH,
    ARKIVE_SECRET_KEY,
    ARKIVE_SESSION_COOKIE_SAME_SITE,
    ARKIVE_SESSION_COOKIE_SECURE,
    ENABLE_SIGNUP_PASSWORD_CONFIRMATION,
    ARKIVE_AUTH_TRUSTED_EMAIL_HEADER,
    ARKIVE_AUTH_TRUSTED_NAME_HEADER,
    ARKIVE_AUTH_SIGNOUT_REDIRECT_URL,
    # SCIM
    ENABLE_SCIM,
    SCIM_TOKEN,
...
from arkive.utils.models import (
    get_all_models,
    get_all_base_models,
    check_model_access,
    get_filtered_models,
)
from arkive.utils.chat import (
    generate_chat_completion as chat_completion_handler,
    chat_completed as chat_completed_handler,
)
from arkive.utils.actions import chat_action as chat_action_handler
from arkive.utils.embeddings import generate_embeddings
from arkive.utils.middleware import (
...
from arkive.utils.plugin import install_tool_and_function_dependencies
from arkive.utils.oauth import (
...
from arkive.utils.security_headers import SecurityHeadersMiddleware
from arkive.utils.redis import get_redis_connection

from arkive.tasks import (
...
from arkive.utils.redis import get_sentinels_from_env
...
async def lifespan(app: FastAPI):
    ...
    if ARKIVE_ADMIN_EMAIL and ARKIVE_ADMIN_PASSWORD:
        if create_admin_user(ARKIVE_ADMIN_EMAIL, ARKIVE_ADMIN_PASSWORD, ARKIVE_ADMIN_NAME):
...
app.state.ARKIVE_NAME = ARKIVE_NAME
...
if ENABLE_OTEL:
    from arkive.utils.telemetry.setup import setup as setup_opentelemetry
...
app.state.config.ARKIVE_URL = ARKIVE_URL
...
app.state.config.BANNERS = ARKIVE_BANNERS
...
from arkive.utils.access_control import migrate_access_control
...
app.state.AUTH_TRUSTED_EMAIL_HEADER = ARKIVE_AUTH_TRUSTED_EMAIL_HEADER
app.state.AUTH_TRUSTED_NAME_HEADER = ARKIVE_AUTH_TRUSTED_NAME_HEADER
app.state.ARKIVE_AUTH_SIGNOUT_REDIRECT_URL = ARKIVE_AUTH_SIGNOUT_REDIRECT_URL
        ...
        if request.method == 'GET':
            ...
            if 'shared' in query_params and len(query_params['shared']) > 0:
                ...
                if text:
                    ...
                    if urls:
                        from arkive.retrieval.loaders.youtube import _parse_video_id
...
# Week 4: Audit trail, confidence scoring, and review management
app.include_router(audit_trail.router, prefix='/api/v1/audit', tags=['audit-trail'])
...
from arkive.utils.anthropic import (
...
async def generate_messages(
    ...
    All models configured in Arkive are accessible via this endpoint.
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout, trust_env=True) as session:
            async with session.get(
                'https://api.github.com/repos/Abduttayyeb07/ArkiveV2IA/releases/latest',
...
async def get_current_usage(user=Depends(get_verified_user)):
    """
    Get current usage statistics for Arkive.
...
try:
    if ENABLE_STAR_SESSIONS_MIDDLEWARE:
        ...
        app.add_middleware(
            ...
            cookie_same_site=ARKIVE_SESSION_COOKIE_SAME_SITE,
            cookie_https_only=ARKIVE_SESSION_COOKIE_SECURE,
        )
    ...
    app.add_middleware(
        SessionMiddleware,
        secret_key=ARKIVE_SECRET_KEY,
        session_cookie='owui-session',
        same_site=ARKIVE_SESSION_COOKIE_SAME_SITE,
        https_only=ARKIVE_SESSION_COOKIE_SECURE,
    )
...
async def get_manifest_json():
    if app.state.EXTERNAL_PWA_MANIFEST_URL:
        ...
        return {
            'name': app.state.ARKIVE_NAME,
            'short_name': app.state.ARKIVE_NAME,
            'description': f'{app.state.ARKIVE_NAME} — Enterprise Intelligence Platform with governed AI reasoning.',
            'start_url': '/',
            'display': 'standalone',
...
async def get_opensearch_xml():
    ...
    <ShortName>{app.state.ARKIVE_NAME}</ShortName>
    <Description>Search {app.state.ARKIVE_NAME}</Description>
    <InputEncoding>UTF-8</InputEncoding>
    <Image width="16" height="16" type="image/x-icon">{app.state.config.ARKIVE_URL}/static/favicon.png</Image>
    <Url type="text/html" method="get" template="{app.state.config.ARKIVE_URL}/?q={'{searchTerms}'}"/>
    <moz:SearchForm>{app.state.config.ARKIVE_URL}</moz:SearchForm>
