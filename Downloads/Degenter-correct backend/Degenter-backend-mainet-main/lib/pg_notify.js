...
export async function pgListen(channel, onMessage) {
  ...
  async function connectAndListen() {
    ...
    const client = runner.databaseConnection;
  ...
  let state = await connectAndListen();

  // Keep-alive / auto-reconnect loop
  (async function keepAlive() {
    while (true) {
      try {
        await state.runner.query('SELECT 1'); // ping
      } catch (e) {
        warn('[pgListen] reconnecting…', e.message);
        try { await state.runner.release(); } catch {}
        state = await connectAndListen();
      }
