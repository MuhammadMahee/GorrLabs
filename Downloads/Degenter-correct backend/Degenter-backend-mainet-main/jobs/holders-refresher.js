 ...
 */
export async function refreshHoldersOnce(
  token_id,
  denom,
  maxPages = MAX_HOLDER_PAGES_PER_CYCLE
) {
  ...
  for (let i = 0; i < maxPages; i++) {
    ...
    try {
      ...
      if (msg.includes('501')) {
        // LCD doesn't support denom_owners for this denom (typical for IBC)
  ...
  try {
    ...
    if (all.length > 0) {
      ...
      // No holders discovered (or early failure) → zero out leftovers
      await client.query(
        `
        UPDATE holders
        SET balance_base = '0', updated_at = now()
 ...
 * - sweep them in parallel (Promise.allSettled)
 * - LCD page concurrency is throttled by Semaphore above
 */
  ...
  (async function loop() {
    while (true) {
      try {
        ...
        if (rows.length > 0) {
          // tokens in this batch are processed in parallel
