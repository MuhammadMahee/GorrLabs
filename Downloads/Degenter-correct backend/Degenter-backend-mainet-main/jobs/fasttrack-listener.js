...
async function loadPoolContext(payload) {
  ...
  const by = payload.pool_id
    ? { col: 'p.pool_id', val: payload.pool_id }
    : payload.pair_contract
    ? { col: 'p.pair_contract', val: payload.pair_contract }
    : null;
  if (!by) return null;
...
// ZIG/USD snapshot helper (same idea as in block-processor)
...
export function startFasttrackListener() {
  ...
  pgListen('pair_created', async (payload) => {
    try {
      ...
      const quoteHC =
        ctx.quote_denom !== 'uzig'
          ? await holdersCount(ctx.quote_token_id)
          : 0;
      ...
      if (baseHC === 0) {
        debug(
          '[fasttrack] base holders 0, retrying once…',
          ctx.base_denom
        );
      ...
      if (ctx.quote_denom !== 'uzig' && quoteHC === 0) {
        debug(
          '[fasttrack] quote holders 0, retrying once…',
          ctx.quote_denom
        );
      ...
      //     including liquidity_zig / liquidity_usd taken from pool_state / FX.
      try {
        if (ctx.quote_denom === 'uzig') {
          ...
          if (baseExp == null) {
            debug('[fasttrack/init skip] meta not ready', {
              pool_id: ctx.pool_id,
              denom: ctx.base_denom,
            });
          } else {
            // reserves at current time (LCD)
            const reserves = await fetchPoolReserves(ctx.pair_contract);
            const priceZig = priceFromReserves_UZIGQuote(
            ...
            if (
              ...
              // Pick bucket based on first provide_liquidity, fallback to created_at
              let bucketTime = ctx.created_at;
              try {
                ...
                if (trows?.[0]?.created_at) {
                  bucketTime = trows[0].created_at;
                }
              } catch (e) {
                warn(
                  '[fasttrack/init-ohlcv] first provide lookup failed',
                  e.message
                );
              }
              const bucket = minuteFloor(bucketTime);

              // load TVL from pool_state (latest)
              let tvlZig = null;
              let tvlUsd = null;
              try {
                const { rows: srows } = await DB.query(
                  `SELECT tvl_zig, tvl_usd
                     FROM pool_state
                    WHERE pool_id = $1`,
                  [ctx.pool_id]
                );
                if (srows?.[0]) {
                  tvlZig =
                    srows[0].tvl_zig != null
                      ? Number(srows[0].tvl_zig)
                      : null;
                  tvlUsd =
                    srows[0].tvl_usd != null
                      ? Number(srows[0].tvl_usd)
                      : null;
                }
              } catch (e) {
                warn(
                  '[fasttrack/init-ohlcv] pool_state lookup failed',
                  e.message
                );
              }

              // FX at that time (for price_usd / liquidity_usd if needed)
              const zigUsd = await getZigUsdForTimestamp(bucket);

              if (zigUsd != null && tvlZig != null && tvlUsd == null) {
                tvlUsd = tvlZig * zigUsd;
              }

              // 6a) Seed ZIG OHLCV: price in ZIG, vol_zig = 0, liquidity_zig = TVL
              await upsertOHLCV1m({
                pool_id: ctx.pool_id,
                bucket_start: bucket,
                price: priceZig,
                ...
                liquidity_zig: tvlZig ?? null,
              });

              // 6b) Seed USD OHLCV: only if we know zigUsd
              if (zigUsd != null) {
                const priceUsd = priceZig * zigUsd;
              ...
              debug('[fasttrack/init-ohlcv]', {
                pool_id: ctx.pool_id,
                bucket: bucket.toISOString(),
                price_zig: priceZig,
              });
            } else {
              debug('[fasttrack/init-ohlcv skip] reserves not ready/non-positive', {
                pool_id: ctx.pool_id,
              });
            }
          }
