...
-- ====================================================================
-- PHASE 1: TRADE SNAPSHOTS (PRICE + VALUE)
-- ====================================================================
-- Existing trades table (from phase3schema.sql) is:
--  CREATE TABLE IF NOT EXISTS public.trades (
--    trade_id BIGSERIAL,
--    pool_id  BIGINT NOT NULL REFERENCES public.pools(pool_id),
--    ...
...
ALTER TABLE public.trades
  ...
  -- helper snapshots
  ADD COLUMN IF NOT EXISTS quote_price_in_zig NUMERIC(38,18), -- quote token → ZIG at trade time
  ADD COLUMN IF NOT EXISTS zig_usd_at_trade   NUMERIC(38,8);  -- ZIG → USD at trade time

-- Optional: if you expect to query by zig_usd_at_trade or price_in_zig a lot,
-- you can add indexes like:
-- CREATE INDEX IF NOT EXISTS idx_trades_price_in_zig
--   ON public.trades(price_in_zig);
-- CREATE INDEX IF NOT EXISTS idx_trades_value_in_zig_time
--   ON public.trades(created_at DESC, value_in_zig);


-- ====================================================================
-- PHASE 2: SECOND OHLCV TABLE IN USD
-- ====================================================================
-- Existing OHLCV table (from phase3schema.sql):
--   public.ohlcv_1m (price in quote, volume_zig, etc.)
--
-- We add a USD version, mainly for ZIG-quoted pools. Non-ZIG pairs can
-- simply never write here (your business rule).

CREATE TABLE IF NOT EXISTS public.ohlcv_1m_usd (
  pool_id        BIGINT NOT NULL REFERENCES public.pools(pool_id),
  bucket_start   TIMESTAMPTZ NOT NULL,
  open           NUMERIC(38,18) NOT NULL,  -- price in USD
  high           NUMERIC(38,18) NOT NULL,
  low            NUMERIC(38,18) NOT NULL,
  close          NUMERIC(38,18) NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_ohlcv_usd_pool_time
  ON public.ohlcv_1m_usd(pool_id, bucket_start DESC);


-- ====================================================================
-- PHASE 3: POOL STATE – TVL IN ZIG + USD
-- ====================================================================
-- Existing table:
--  CREATE TABLE IF NOT EXISTS public.pool_state (
--    pool_id            BIGINT PRIMARY KEY REFERENCES public.pools(pool_id),
...
-- tvl_zig  = total pool value in ZIG (base + quote converted to ZIG).
-- tvl_usd  = tvl_zig * zig_usd_at_snapshot.


-- ====================================================================
-- PHASE 4: IBC METADATA (MAPPING TO CMC / COINGECKO)
-- ====================================================================
-- tokens table already has:
--  type (enum 'native','factory','ibc','cw20'),
--  max_supply_base, total_supply_base, etc.
--
-- This table is for extra metadata / mapping only.

CREATE TABLE IF NOT EXISTS public.ibc_tokens (
  token_id        BIGINT PRIMARY KEY
                  REFERENCES public.tokens(token_id) ON DELETE CASCADE,
  base_denom      TEXT,   -- e.g. 'wbtc', 'uatom'
  source_chain    TEXT,   -- e.g. 'cosmoshub-4', 'evmos_9001-2'
  source_channel  TEXT,   -- e.g. 'channel-0'
  coingecko_id    TEXT,
  cmc_id          TEXT,
  updated_at      TIMESTAMPTZ NOT NULL DEFAULT now()
);

CREATE INDEX IF NOT EXISTS idx_ibc_tokens_base_denom
  ON public.ibc_tokens(base_denom);

CREATE INDEX IF NOT EXISTS idx_ibc_tokens_cmc_id
  ON public.ibc_tokens(cmc_id);

CREATE INDEX IF NOT EXISTS idx_ibc_tokens_coingecko_id
  ON public.ibc_tokens(coingecko_id);


-- ====================================================================
-- PHASE 5: EXTERNAL PRICES (OPTIONAL, GLOBAL REFERENCE)
-- ====================================================================
-- This is NOT required for core DEX pricing (that still comes from pools).
-- It's useful if you want to store CMC/Coingecko global prices separately
-- to compare against on-chain prices or to drive “global” market cap, etc.

CREATE TABLE IF NOT EXISTS public.external_prices (
  token_id    BIGINT NOT NULL REFERENCES public.tokens(token_id)
              ON DELETE CASCADE,
  source      TEXT   NOT NULL,  -- 'cmc', 'coingecko', 'custom', etc.
  price_usd   NUMERIC(38,18) NOT NULL,
  updated_at  TIMESTAMPTZ NOT NULL DEFAULT now(),
  PRIMARY KEY (token_id, source)
);

CREATE INDEX IF NOT EXISTS idx_external_prices_updated
  ON public.external_prices(updated_at DESC);
...
CREATE TABLE IF NOT EXISTS ibc_holder_candidates (
