...
const UPSERT_SQL = `
  INSERT INTO pool_state(pool_id, reserve_base_base, reserve_quote_base, tvl_zig, tvl_usd, updated_at)
  VALUES %VALUES%
...
function sqlValues(rows) {
  ...
  let i = 1;
  for (const r of rows) {
    vals.push(`($${i++},$${i++},$${i++},$${i++},$${i++}, now())`);
    args.push(
      r.pool_id,
      r.reserve_base_base,
      r.reserve_quote_base,
      r.tvl_zig ?? null,
      r.tvl_usd ?? null
    );
  }
...
// tvl_zig / tvl_usd are optional snapshot values computed upstream
export async function upsertPoolState(
  ...
  if (res1d === baseDenom && res2d === quoteDenom) {
    base = res1a;
    quote = res2a;
  } else if (res2d === baseDenom && res1d === quoteDenom) {
    base = res2a;
    quote = res1a;
  }
