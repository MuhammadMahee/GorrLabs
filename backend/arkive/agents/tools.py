...
LOW_QUALITY_THRESHOLD = float(os.getenv("LOW_QUALITY_THRESHOLD", "0.5"))
...
def _score_sources(sources: list[dict]) -> float:
    """
    Returns the average reranker score across all chunks in sources.
    Score is stored in metadata['score'] by RerankCompressor.acompress_documents().
    Returns 0.0 when no scores are present (e.g. reranker not configured).
    """
    scores: list[float] = []
    for source in sources:
...
async def search_knowledge_base(
    ...
    try:
        ...
        avg_score = _score_sources(filtered)
        quality = (
            ToolStatus.LOW_QUALITY
            if avg_score > 0.0 and avg_score < LOW_QUALITY_THRESHOLD
            else ToolStatus.SUCCESS
        )
        if quality == ToolStatus.LOW_QUALITY:
            log.info(
                f"[agent_tools.search_knowledge_base] low quality "
                f"avg_score={avg_score:.3f} threshold={LOW_QUALITY_THRESHOLD}"
            )
        ...
        return ToolResult(
            ...
            chunks_used=count,
            metadata={"collections_searched": names, "limit": limit, "avg_score": avg_score},
        )
