...
from arkive.utils.misc import is_string_allowed
from arkive.models.oauth_sessions import OAuthSessions
from arkive.models.chats import Chats
from arkive.models.folders import Folders
from arkive.models.users import Users
from arkive.socket.main import (
    get_event_call,
    get_event_emitter,
)
from arkive.routers.tasks import (
    generate_queries,
    generate_title,
    generate_follow_ups,
...
from arkive.utils.tools import get_builtin_tools
from arkive.routers.images import (
    image_generations,
    CreateImageForm,
    image_edits,
    EditImageForm,
...
from arkive.routers.memories import query_memory, QueryMemoryForm

from arkive.utils.webhook import post_webhook
from arkive.utils.files import (
...
from arkive.models.users import UserModel
from arkive.models.functions import Functions
from arkive.models.models import Models

from arkive.retrieval.utils import get_sources_from_items


from arkive.utils.sanitize import sanitize_code
from arkive.utils.chat import generate_chat_completion
from arkive.utils.task import (
    get_task_model_id,
    rag_template,
...
from arkive.utils.tools import (
    get_tools,
    get_updated_tool_function,
    get_terminal_tools,
)
from arkive.utils.access_control import has_connection_access
from arkive.utils.plugin import load_function_module_by_id
from arkive.utils.filter import (
    get_sorted_filter_ids,
    process_filter_functions,
)
from arkive.utils.code_interpreter import execute_code_jupyter
...
from arkive.utils.headers import include_user_info_headers
from arkive.constants import TASKS
...
def serialize_output(output: list) -> str:
    ...
    for idx, item in enumerate(output):
        ...
        elif item_type == 'arkive:code_interpreter':
...
async def process_chat_payload(request, form_data, user, metadata, model):
    ...
    available_skills = []
    if all_skill_ids:
        from arkive.models.skills import Skills as SkillsModel
            ...
            if choices and choices[0].get('message', {}).get('content'):
                ...
                if content:
                    ...
                    if request.app.state.config.ENABLE_USER_WEBHOOKS and not Users.is_user_active(user.id):
                        ...
                        if webhook_url:
                            await post_webhook(
                                request.app.state.ARKIVE_NAME,
                                webhook_url,
                                f'{title} - {request.app.state.config.ARKIVE_URL}/c/{metadata["chat_id"]}\n\n{content}',
                                {
                                    'action': 'chat',
                                    'message': content,
                                    'title': title,
                                    'url': f'{request.app.state.config.ARKIVE_URL}/c/{metadata["chat_id"]}',
                                },
                            )
        ...
        async def response_handler(response, events):
            def tag_output_handler(content_type, tags, output):
                ...
                output_type_map = {
                    ...
                    'code_interpreter': 'arkive:code_interpreter',
                }
                ...
                if last_type == 'message':
                    ...
                    for start_tag, end_tag in tags:
                        ...
                        if match:
                            ...
                            elif output_item_type == 'arkive:code_interpreter':
                                output.append(
                                    {
                                        'type': 'arkive:code_interpreter',
                                        'id': output_id('ci'),
                                        'status': 'in_progress',
                                        'start_tag': start_tag,
                                        'end_tag': end_tag,
                    ...
                    or (last_type == 'arkive:code_interpreter' and content_type == 'code_interpreter')
                    ...
                    elif last_type == 'arkive:code_interpreter':
            ...
            try:
                                    ...
                                    if value:
                                        ...
                                        inside_tag_block = (
                                            ...
                                            and (
                                                ...
                                                or last_item_type == 'arkive:code_interpreter'
                                                or (
                                        ...
                                        if inside_tag_block:
                                            ...
                                            if last_item_type == 'arkive:code_interpreter':
                ...
                if DETECT_CODE_INTERPRETER:
                    ...
                    while output and output[-1].get('type') == 'arkive:code_interpreter' and retries < MAX_RETRIES:
                        await event_emitter(
                            {
                                'type': 'chat:completion',
                                'data': {
