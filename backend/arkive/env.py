...
# ARKIVE_DIR — directory where env.py resides (backend/arkive/)
ARKIVE_DIR = ENV_FILE_PATH.parent

# BACKEND_DIR — parent of ARKIVE_DIR (backend/)
BACKEND_DIR = ARKIVE_DIR.parent

# BASE_DIR is the parent of BACKEND_DIR (arkive-dev/)
...
ARKIVE_NAME = os.environ.get('ARKIVE_NAME', 'Arkive')

ARKIVE_FAVICON_URL = '/static/favicon.svg'
...
if FROM_INIT_PY:
    PACKAGE_DATA = {'version': importlib.metadata.version('arkive')}
else:
    try:
...
FORWARD_USER_INFO_HEADER_USER_NAME = os.environ.get('FORWARD_USER_INFO_HEADER_USER_NAME', 'X-Arkive-User-Name')
FORWARD_USER_INFO_HEADER_USER_ID = os.environ.get('FORWARD_USER_INFO_HEADER_USER_ID', 'X-Arkive-User-Id')
FORWARD_USER_INFO_HEADER_USER_EMAIL = os.environ.get('FORWARD_USER_INFO_HEADER_USER_EMAIL', 'X-Arkive-User-Email')
FORWARD_USER_INFO_HEADER_USER_ROLE = os.environ.get('FORWARD_USER_INFO_HEADER_USER_ROLE', 'X-Arkive-User-Role')
...
FORWARD_SESSION_INFO_HEADER_MESSAGE_ID = os.environ.get(
    'FORWARD_SESSION_INFO_HEADER_MESSAGE_ID', 'X-Arkive-Message-Id'
)
FORWARD_SESSION_INFO_HEADER_CHAT_ID = os.environ.get('FORWARD_SESSION_INFO_HEADER_CHAT_ID', 'X-Arkive-Chat-Id')
...
ARKIVE_BUILD_HASH = os.environ.get('ARKIVE_BUILD_HASH', 'dev-build')
...
if FROM_INIT_PY:
    NEW_DATA_DIR = Path(os.getenv('DATA_DIR', ARKIVE_DIR / 'data')).resolve()
    ...
    if DATA_DIR.exists() and DATA_DIR != NEW_DATA_DIR:
        ...
        shutil.make_archive(DATA_DIR.parent / 'arkive_data', 'zip', DATA_DIR)

        # Remove the old data directory
        shutil.rmtree(DATA_DIR)

    DATA_DIR = Path(os.getenv('DATA_DIR', ARKIVE_DIR / 'data'))

STATIC_DIR = Path(os.getenv('STATIC_DIR', ARKIVE_DIR / 'static'))

FONTS_DIR = Path(os.getenv('FONTS_DIR', ARKIVE_DIR / 'static' / 'fonts'))

FRONTEND_BUILD_DIR = Path(os.getenv('FRONTEND_BUILD_DIR', BASE_DIR / 'build')).resolve()

if FROM_INIT_PY:
    FRONTEND_BUILD_DIR = Path(os.getenv('FRONTEND_BUILD_DIR', ARKIVE_DIR / 'frontend')).resolve()
...
if os.path.exists(f'{DATA_DIR}/ollama.db'):
    # Rename the file
    os.rename(f'{DATA_DIR}/ollama.db', f'{DATA_DIR}/arkive.db')
    log.info('Database migrated from Ollama-Arkive successfully.')
else:
    pass

DATABASE_URL = os.environ.get('DATABASE_URL', f'sqlite:///{DATA_DIR}/arkive.db')
...
if all(DB_VARS.values()):
    ...
    DATABASE_URL = f'sqlite+sqlcipher:///{DATA_DIR}/arkive.db'
...
REDIS_KEY_PREFIX = os.environ.get('REDIS_KEY_PREFIX', 'arkive')
...
####################################
# ARKIVE_AUTH (Required for security)
####################################

ARKIVE_AUTH = os.environ.get('ARKIVE_AUTH', 'True').lower() == 'true'
...
ARKIVE_ADMIN_EMAIL = os.environ.get('ARKIVE_ADMIN_EMAIL', '')
ARKIVE_ADMIN_PASSWORD = os.environ.get('ARKIVE_ADMIN_PASSWORD', '')
ARKIVE_ADMIN_NAME = os.environ.get('ARKIVE_ADMIN_NAME', 'Admin')

ARKIVE_AUTH_TRUSTED_EMAIL_HEADER = os.environ.get('ARKIVE_AUTH_TRUSTED_EMAIL_HEADER', None)
ARKIVE_AUTH_TRUSTED_NAME_HEADER = os.environ.get('ARKIVE_AUTH_TRUSTED_NAME_HEADER', None)
ARKIVE_AUTH_TRUSTED_GROUPS_HEADER = os.environ.get('ARKIVE_AUTH_TRUSTED_GROUPS_HEADER', None)
ARKIVE_AUTH_TRUSTED_ROLE_HEADER = os.environ.get('ARKIVE_AUTH_TRUSTED_ROLE_HEADER', None)
...
ARKIVE_AUTH_SIGNOUT_REDIRECT_URL = os.environ.get('ARKIVE_AUTH_SIGNOUT_REDIRECT_URL', None)

####################################
# ARKIVE_SECRET_KEY
####################################

ARKIVE_SECRET_KEY = os.environ.get(
    'ARKIVE_SECRET_KEY',
    os.environ.get('ARKIVE_JWT_SECRET_KEY', 't0p-s3cr3t'),  # DEPRECATED: remove at next major version
)

ARKIVE_SESSION_COOKIE_SAME_SITE = os.environ.get('ARKIVE_SESSION_COOKIE_SAME_SITE', 'lax')

ARKIVE_SESSION_COOKIE_SECURE = os.environ.get('ARKIVE_SESSION_COOKIE_SECURE', 'false').lower() == 'true'

ARKIVE_AUTH_COOKIE_SAME_SITE = os.environ.get('ARKIVE_AUTH_COOKIE_SAME_SITE', ARKIVE_SESSION_COOKIE_SAME_SITE)

ARKIVE_AUTH_COOKIE_SECURE = (
    os.environ.get(
        'ARKIVE_AUTH_COOKIE_SECURE',
        os.environ.get('ARKIVE_SESSION_COOKIE_SECURE', 'false'),
    ).lower()
    == 'true'
)

if ARKIVE_AUTH and ARKIVE_SECRET_KEY == '':
...
OAUTH_CLIENT_INFO_ENCRYPTION_KEY = os.environ.get('OAUTH_CLIENT_INFO_ENCRYPTION_KEY', ARKIVE_SECRET_KEY)

OAUTH_SESSION_TOKEN_ENCRYPTION_KEY = os.environ.get('OAUTH_SESSION_TOKEN_ENCRYPTION_KEY', ARKIVE_SECRET_KEY)
...
# Allows external apps to exchange OAuth tokens for Arkive tokens
...
OTEL_SERVICE_NAME = os.environ.get('OTEL_SERVICE_NAME', 'arkive')
